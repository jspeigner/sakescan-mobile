...
export default function Index() {
  const { user, isGuest, isLoading, isPasswordRecovery } = useAuth();
  ...
  useEffect(() => {
    ...
    const dest = isPasswordRecovery
      ? '/reset-password'
      : (user || isGuest) ? '/(tabs)' : '/welcome';
    console.log('[Index] navigating to:', dest);
    router.replace(dest as Parameters<typeof router.replace>[0]);
  }, [user, isGuest, isLoading, navReady, isPasswordRecovery]);
