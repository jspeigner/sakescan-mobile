...
- Supabase URL and anon key are hardcoded as defaults in `src/lib/supabase.ts`, so browsing/searching the live sake catalog works with no `.env` and no secrets. Label/menu scanning needs the Edge Functions deployed with server secret `OPENAI_API_KEY` (never `EXPO_PUBLIC_OPENAI_*`). Optional: WineEngine cascade (`EXPO_PUBLIC_WINE_ENGINE_ENABLED` + deployed Edge Functions). Camera scanning also can't run in web/headless anyway.
